...
এই doc এ আমাদের Training Institute backend project এ Redis client কিভাবে setup করবো তা step-by-step দেখাবো।
...
```
server.TrainingInstituteManagement/
├── .env                                    ← REDIS_URL add
└── src/
    └── app/
        ├── config/
...
const redis = new Redis(config.redis_url, {
  ...
  connectionName: "training-institute-backend",
});
