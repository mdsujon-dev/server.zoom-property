...
```bash
cd c:/project/trainingInstituteManagement/server.TrainingInstituteManagement
npm install ioredis
```
...
```bash
docker run -d --name training-institute-redis -p 6379:6379 redis:alpine
```

**Command explain:**
- `-d` → background এ run
- `--name training-institute-redis` → container এর নাম
...
# Log দেখতে
docker logs training-institute-redis

# Stop
docker stop training-institute-redis

# আবার start
docker start training-institute-redis

# Remove
docker rm -f training-institute-redis
```

**Data persistent রাখতে (recommended):**

```bash
docker run -d --name training-institute-redis \
  -p 6379:6379 \
  -v training-institute-redis-data:/data \
...
```bash
docker exec -it training-institute-redis redis-cli ping
```
