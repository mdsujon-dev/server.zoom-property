...
const loginUser = async (payload: IAuth) => {
  ...
  return {
    ...
    user: {
      ...
      // The panel puts a change-password dialog in front of everything while
      // this is true — the account is still on the password it was issued.
      is_password_change: Boolean(user.isPasswordChange),
    },
  };
};
...
const changePassword = async (
  ...
  /**
   * An account still on the password it was issued with may replace it without
   * typing that password back.
   *
   * Asking for it proves nothing there: it is the company-wide default, the
   * person was handed it a minute ago at the desk, and every colleague can
   * guess it — so the question is friction standing between a public account
   * and a private one.
   *
   * For every other account the current password is still required, and that is
   * not negotiable: without it, anyone who got hold of a live session could set
   * a new password and keep the account for good.
   */
  const stillOnIssuedPassword = Boolean(user.isPasswordChange);

  if (!stillOnIssuedPassword) {
    if (!oldPassword) {
      throw new AppError(
  ...
  // Clearing the flag here is what releases the panel: whatever the account
  // started on, it is no longer on the issued default.
  await User.updateOne(
    { _id: userId },
    { password: hashedPassword, isPasswordChange: false }
  );
...
<div style="background:#ffffff;font-family:'Outfit','Segoe UI',Arial,sans-serif;color:#1f2937;margin:0;padding:0;width:100%;">
  <div style="width:100%;background:linear-gradient(135deg,#133050 0%,#0a1b2d 100%);padding:32px 24px;color:#ffffff;box-sizing:border-box;">
    <p style="margin:0;font-size:12px;letter-spacing:0.18em;text-transform:uppercase;opacity:0.85;">Zoom Property — Password Reset</p>
...
const forgotPassword = async (email: string) => {
  ...
  try {
    await EmailHelper.sendEmail(
      ...
      "Your Zoom Property password reset code"
    );
  } catch (err) {
    console.error("[AuthService.forgotPassword] email send failed:", err);
    // Don't surface the failure to the lead — we already wrote the code.
...
const resetPassword = async (input: {
  ...
  await User.updateOne(
    ...
    {
      password: hashed,
      // Resetting through email counts as replacing the issued default.
