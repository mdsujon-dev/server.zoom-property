...
import { isPersonaRole } from "../../access";
...
/**
 * FACULTY and STUDENT read their access from `src/app/access`, not from here.
 * A row written against one of them would be stored and then ignored, which is
 * worse than being refused: the panel would show a permission the API does not
 * honour.
 */
const assertEditableRole = async (roleId: unknown) => {
  const role = await Role.findById(roleId);
  if (!role) {
    throw new AppError(StatusCodes.NOT_FOUND, "Role not found");
  }
  if (isPersonaRole(role.role)) {
    throw new AppError(
      StatusCodes.FORBIDDEN,
      `${role.role} access is predefined in code and cannot be granted here`
    );
  }
  return role;
};
...
const createRolePermission = async (payload: Partial<IRolePermission>) => {
  await assertEditableRole(payload.roleId);
...
const updateRolePermission = async (
  ...
  await assertEditableRole(payload.roleId ?? existing.roleId);
