...
/**
 * Streams the file itself rather than describing it.
 *
 * Bypasses `sendResponse` on purpose — this one answers with bytes, not the
 * usual JSON envelope, because the caller is an `<img>` or a `fetch` that wants
 * to read the pixels.
 */
const getMediaFile = catchAsync(async (req, res) => {
  const file = await MediaLibraryServices.getMediaFile(req.params.id);

  res.setHeader("Content-Type", file.contentType);
  if (file.length) res.setHeader("Content-Length", String(file.length));
  // The bytes behind an id never change — a new upload gets a new id — so this
  // can be cached hard, which keeps the PDF button from re-downloading a logo
