...
const getMediaUsage = async (id: string) => {
  ...
  // Resolve media URL for schemas that store image URLs
...
/**
 * The raw bytes of one media object, streamed through this API.
 *
 * The public R2 URL answers without an `Access-Control-Allow-Origin` header,
 * which browsers accept for *showing* a picture and refuse for *reading* one.
 * So a logo displays perfectly on screen and then vanishes from the certificate
 * PDF — html2canvas has to read the pixels, and is not allowed to.
 *
 * Served from here, the bytes come back under this API's own CORS policy, which
 * the admin panel is already on. A CORS rule on the bucket would be the tidier
 * fix, but that is a setting in somebody's Cloudflare dashboard, and a feature
 * should not quietly depend on a click nobody has made.
 */
const getMediaFile = async (id: string) => {
  const media = await Media.findOne({ _id: id, is_deleted: { $ne: true } });
  if (!media) throw new AppError(StatusCodes.NOT_FOUND, "Media not found");

  const object = await r2.send(
    new GetObjectCommand({ Bucket: R2_BUCKET, Key: media.key })
  );

  return {
    body: object.Body as NodeJS.ReadableStream,
    contentType: object.ContentType || media.type || "application/octet-stream",
