...
export const actionLogger = (
  ...
  let leadDetails: Record<string, string> = {};
  const rawLeadDetails = headers["x-lead-details"];
  if (typeof rawLeadDetails === "string") {
    try {
      leadDetails = JSON.parse(rawLeadDetails) ?? {};
    } catch {
      // Silently ignore malformed x-lead-details; other fields still get logged.
    }
  }
  ...
  res.on("finish", () => {
    ...
    const payload = {
      ...
      leadDetails: {
        ipAddress: leadDetails?.ipAddress || ipAddress,
        userAgent: leadDetails?.userAgent || userAgent,
        browserUrl: leadDetails?.browserUrl || "",
        accessedAt: leadDetails?.accessedAt || new Date().toISOString(),
      },
      serverDetails: {
